  ...
  return Promise.all(
    pages.map(page => {
      ...
      if (template === 'Default') {
        ...
        const components =
          page.template[options.graphQLFieldGroupName][
        ...
        createPageArgs.component = createTemporaryPageTemplateFile(
          page.databaseId,
          page.nodeType,
          page.slug,
      ...
      return gatsbyUtilities.actions.createPage(createPageArgs)
    })
  )
}

const getRootId = page => {
  let root = page
  while (root.wpParent !== null) {
    root = root.wpParent.node
  }
  return root.id
}
...
const createTemporaryPageTemplateFile = (
  ...
  const componentsVariableString = `
    data = pageProps.data.wp${postType}
    components = data['${options.graphQLFieldGroupName}']['${options.graphQLFieldName}'] || []
  `
  ...
  const componentRenderString = componentNames
    .map(componentName => {
      ...
      if (component.name === '${componentName}') {
        return <${componentName} {...component.data} key={index} pageLocation={pageProps.location} />
      }
    `
    })
    .join('\n')
...
const getComponentFragments = (
  ...
  componentNames.forEach(componentName => {
    ...
    if (existsSync(componentFragmentFile)) {
      query = require(componentFragmentFile)()
    } else {
      query = '__typename'
    }
